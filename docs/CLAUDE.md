...
- **Model loading must free before load.** `ModelManager.loadModel()` is the single entry point for all LLM loading. It unloads all previous backends and nils `modelContainer` before loading the new model — this prevents two models coexisting in memory (OOM on 8/16 GB machines). Never load a model by calling `loadMLXModel()` directly, and never register a backend on `InferenceRouter` without removing the old one first. Settings UI that triggers a reload (e.g., vision toggle, model picker) must go through `reloadLLM()` → `loadModel()`, not bypass it.
- **Factory selection is performance-critical.** `ModelManager.loadMLXModel` computes `useVLM = model.isVLM && visionEnabled` to choose between `VLMModelFactory.shared` and `LLMModelFactory.shared`. Both Qwen 3.5 and Gemma 4 are native-multimodal VLM checkpoints but still have a usable text-only path because `LLMModelFactory` weight-sanitization strips the vision tower. The text-only `LLMModelFactory` path is **measurably faster** for prompts without images. **Never** drop the `&& visionEnabled` gate — users rely on toggling "Allow vision" off to get the fast path. This applies equally to Qwen 3.5 and Gemma 4: they share the same routing logic.
- **`mlx-swift-lm` is pinned to an exact upstream commit**, not a version range. The pin lives in two places (`HitokuInference/Package.swift` and `project.yml`) and must stay in sync. Bumping procedure is documented in `docs/gemma4-migration.md`. Do not switch to `.upToNextMinor(from:)` until upstream cuts a stable release that covers Gemma 4 + tool calling + audio. The `adrgrondin/mlx-swift-lm` fork must **not** be re-introduced — it had a `broadcast_shapes` crash in the Gemma 4 repetition-penalty processor.
